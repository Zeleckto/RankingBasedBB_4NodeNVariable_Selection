...
def extract_bipartite_graph(model):
    ...
    obj_val = model.getObjVal() if model.getNSols() > 0 else 1.0
    obj_val = obj_val if abs(obj_val) > 1e-8 else 1.0
    ...
    for i, row in enumerate(rows):
        ...
        # 2: dual solution value (normalized by obj)
        con_feats[i, 2] = dualsol / (abs(obj_val) + 1e-8)

        # 3: LP age (normalized) — defensive, not all versions expose this
        try:
    ...
    for j, col in enumerate(cols):
        ...
        # 4: objective coefficient normalized
        var_feats[j, 4] = obj_c / (abs(obj_val) + 1e-8)
...
def get_prenorm_stats(feature_dicts):
    """
    Compute prenorm stats (mean, std) from a list of feature dicts.
    Used to initialize prenorm layers in GCN before training.
    """
    all_con = np.concatenate([d["con_feats"] for d in feature_dicts], axis=0)
    all_var = np.concatenate([d["var_feats"] for d in feature_dicts], axis=0)
    all_edg = np.concatenate([d["edge_feats"] for d in feature_dicts], axis=0)

    return {
        "con_mean": all_con.mean(0).astype(np.float32),
        "con_std":  (all_con.std(0) + 1e-8).astype(np.float32),
        "var_mean": all_var.mean(0).astype(np.float32),
        "var_std":  (all_var.std(0) + 1e-8).astype(np.float32),
        "edg_mean": all_edg.mean(0).astype(np.float32),
        "edg_std":  (all_edg.std(0) + 1e-8).astype(np.float32),
    }